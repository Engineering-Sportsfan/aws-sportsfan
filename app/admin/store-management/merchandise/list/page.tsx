...
export default function MerchandiseListPage() {
  ...
  const fetchMerchandise = async () => {
    ...
    setError("");
    try {
      const res = await axios.get("/api/admin/store/addMerchandise");
      if (res.data.success) {
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/store-management/merchandise/add?id=${id}`);
  };
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      const res = await axios.delete(`/api/admin/store/addMerchandise?id=${id}`);
      if (res.data.success) {
  ...
  return (
    <div className="max-w-[1440px] mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-semibold text-white">Merchandise</h1>
        <button
          onClick={() => router.push("/admin/store-management/merchandise/add")}
      ...
      {loading ? (
        <div className="text-white">Loading merchandise...</div>
      ) : error ? (
        <div className="text-red-500">
          <p>{error}</p>
          <button onClick={fetchMerchandise} className="mt-2 bg-gray-800 px-4 py-2 rounded text-sm text-white hover:bg-gray-700">Retry</button>
        </div>
      ) : merchandise.length === 0 ? (
        <div className="text-gray-400">No merchandise found.</div>
      ) : (
            ...
            <tbody>
              {merchandise.map((item) => (
                <tr key={item.id} className="border-b border-[#21262d] hover:bg-[#0d1117]/50">
                  ...
                  <td className="px-4 py-3">{item.athlete}</td>
