// app/api/team360-playlist/[id]/route.ts — Migrated to AWS DynamoDB (SportsData Table)
...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn("[team360-playlist [id] GET] DynamoDB notice:", e);
    }

    // 2. Fallback to Firestore
    if (db) {
      const doc = await db.collection("team360Playlists").doc(id).get();
      if (doc.exists) {
        return NextResponse.json({
          success: true,
          playlist: {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn("[team360-playlist [id] PUT] DynamoDB check:", e);
    }
    ...
    let team360PostId: string = (existingData.team360PostId as string) || "";
    ...
    if (isFormData) {
      ...
      if (body.team360PostId !== undefined) team360PostId = body.team360PostId;
    ...
    const dynamoItem = {
      entityId: `TEAM360_PLAYLIST#${id}`,
      sk: `PLAYLIST#${team360PostId || "META"}`,
      ...updatedData,
    };
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await dualDelete("team360Playlists", id, "SportsData", {
      entityId: `TEAM360_PLAYLIST#${id}`,
      sk: "PLAYLIST#META",
    });
