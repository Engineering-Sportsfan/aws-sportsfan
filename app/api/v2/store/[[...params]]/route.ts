...
export async function POST(
  ...
  try {
    ...
    // 6. PAYMENTS (RAZORPAY)
    if (firstSegment === 'payments' && pathParams.length >= 3 && pathParams[1] === 'razorpay') {
      console.log("first segment", firstSegment);
      const subAction = pathParams[2];
      ...
      if (subAction === 'createOrder') {
        const { productId, slotId, variantId, userId, idempotencyKey } = body;
      ...
      if (subAction === 'verify') {
        ...
        try {
          ...
          return NextResponse.json({
            success: false,
            error: `Post-payment registration failed: ${checkoutError.message || 'unknown error'}. Please contact support with your payment ID (${razorpay_payment_id}) for manual confirmation or refund.`,
