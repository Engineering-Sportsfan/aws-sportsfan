// api/cricket-stats/route.ts — Migrated to AWS DynamoDB (SportsData Table)
  ...
  try {
    ...
    try {
      ...
      if (scanRes.Items && scanRes.Items.length > 0) {
        matches = scanRes.Items as MatchDoc[];
      }
    } catch (e) {
      console.warn("[cricket-stats GET] DynamoDB notice:", e);
    }
    ...
    matches.forEach((m) => {
      ...
      seasonsMap[seasonValue].total += 1;
      if (!getBoolean(m.isNoResult)) {
        seasonsMap[seasonValue].completed += 1;
      }
    });

    const seasons = Object.entries(seasonsMap).map(([seasonKey, data]) => ({
      season: seasonKey,
      total: data.total,
    ...
    return NextResponse.json(
      {
        ...
        highestTotal,
        topVenues: topVenues.map(([venue, count]) => ({ venue, count })),
        seasons,
      },
      { status: 200 }
    );
    ...
    console.error("Error generating cricket stats:", error);
